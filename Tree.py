...
filepath = './Mydata/Credit Card Default II (balance).xlsx'
...
def get_data(filename):
    data = pd.read_excel(filepath, sheetname='Credit Card Default II (balance')
    #print("prezatation")
    print(data)
    data = np.array(data)
    return data
...
def show_data(data):
    ...
    for i in temple:
        if i[-1] == 0:
            ax1.scatter3D(i[0], i[1], i[2], marker='o', color='r')
        else:
            ax1.scatter3D(i[0], i[1], i[2], marker='>', color='b')
